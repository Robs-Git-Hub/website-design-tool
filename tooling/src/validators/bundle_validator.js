...
function loadValidationData(dataDir) {
    // Load Layer 1 dimensions
    const layer1Path = path.join(dataDir, 'layer1_dimensions_instances.toml');
    const layer1Data = TOML.parse(fs.readFileSync(layer1Path, 'utf-8'));
    const layer1Values = new Map();
    if (Array.isArray(layer1Data.dimensions)) {
        for (const dimension of layer1Data.dimensions) {
    ...
    const layer2Data = TOML.parse(fs.readFileSync(layer2Path, 'utf-8'));
    const layer2StyleIds = new Set();
    ...
    const layer3Data = TOML.parse(fs.readFileSync(layer3Path, 'utf-8'));
    const layer3LexiconIds = new Set();
    ...
    const layer4Data = TOML.parse(fs.readFileSync(layer4Path, 'utf-8'));
    const layer4TrendIds = new Set();
...
class ValidationResult {
    errors = [];
    warnings = [];
    addError(path, message, type = 'schema_violation') {
        this.errors.push({ type, path, message, severity: 'error' });
    }
    addWarning(path, message, type = 'schema_violation') {
...
export class WASBundleValidator {
    ...
    validateLayer1(layer1, result) {
        ...
        for (const axis of simpleAxes) {
            ...
            if (!validValues.has(value)) {
                result.addError(`layer1_axes.${axis}`, `Invalid value '${value}'. Valid values: ${Array.from(validValues).join(', ')}`);
            }
        }
        ...
        if (layer1.geometry_depth) {
            ...
            if (shapeValidValues && !shapeValidValues.has(layer1.geometry_depth.shape)) {
                result.addError('layer1_axes.geometry_depth.shape', `Invalid value '${layer1.geometry_depth.shape}'. Valid values: ${Array.from(shapeValidValues).join(', ')}`);
            }
            // Validate depth
            ...
            if (depthValidValues && !depthValidValues.has(layer1.geometry_depth.depth)) {
                result.addError('layer1_axes.geometry_depth.depth', `Invalid value '${layer1.geometry_depth.depth}'. Valid values: ${Array.from(depthValidValues).join(', ')}`);
            }
        }
    }
    validateLayer2(layer2, result) {
        for (const [styleId, weight] of Object.entries(layer2)) {
            // Validate ID exists
            if (!this.validationData.layer2StyleIds.has(styleId)) {
                result.addError(`layer2_styles.${styleId}`, `Style ID '${styleId}' not found in layer2_website_style_family_instances.toml`);
            }
            // Validate weight range (already done by Zod, but we can add a warning for edge cases)
            if (typeof weight === 'number' && (weight === 0 || weight === 1)) {
                result.addWarning(`layer2_styles.${styleId}`, `Weight is ${weight}. Consider if this extreme value is intentional.`);
            }
        }
    ...
    validateLayer3(layer3, result) {
        ...
        for (const [kind, lexiconId] of Object.entries(layer3)) {
            ...
            // Validate ID exists
            if (!this.validationData.layer3LexiconIds.has(lexiconId)) {
                result.addError(`layer3_lexicon.${kind}`, `Lexicon ID '${lexiconId}' not found in layer3_lexicon_instances.toml`);
            }
        }
    ...
    validateLayer4(layer4, result) {
        for (const [trendCategory, trendId] of Object.entries(layer4)) {
            // Validate ID exists
            if (!this.validationData.layer4TrendIds.has(trendId)) {
                result.addError(`layer4_trends.${trendCategory}`, `Trend ID '${trendId}' not found in layer4_societal_trends_instances.toml`);
            }
        }
    }
    /**
     ...
     */
    static formatErrorsForLLM(errors) {
...
// CLI
// ============================================================================
function printValidationResult(result, bundlePath) {
    console.log('\n' + '='.repeat(80));
...
//# sourceMappingURL=bundle_validator.js.map